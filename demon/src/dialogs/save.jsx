...
export default class Save extends Component {
  ...
  async handleSave(no) {
    const ret = await new Promise((resolve, reject) => {
      this.setState({ confirmText: `Save to archive No. ${no + 1}?`, confirmCallback: resolve, alpha: 1 });
    });

    if (ret) {
      const name = `archive-${no + 1}`;
      const text = '';
      const shotData = core.plugins.shot.get();

      await core.post('save-archive', { name, extra: { text, shot: shotData } });
